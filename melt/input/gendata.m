clear
folder = '/media/toshiki/HDD_hirata1/run/jim_run3/run/';
file = dir(fullfile(folder, 'surfDiag.*.data'));

melt = zeros(60, 100, 3*12*60);
k = 1 ;

for i = 1 : 3*12*60
    str = strcat(folder,'surfDiag');
    su = rdmds(str, str2double(file(i).name(11:end-5)));    s = -su(:,:,1)/920*86400*365; 
%     su2 = rdmds(str, str2double(file(i+1).name(11:end-5))); s2 = -su2(:,:,1)/920*86400*365; 
%     su3 = rdmds(str, str2double(file(i+2).name(11:end-5))); s3 = -su3(:,:,1)/920*86400*365; 
...
fid = fopen('meltCTRL.bin','w','b'); fwrite(fid,melt,'real*8'); fclose(fid);
...
%%
clear
s = zeros(60,100);  s(30,2) = 0.15;


fid = fopen('subglflux.bin','w','b'); fwrite(fid,s,'real*8'); fclose(fid);
%%
clear
a = zeros(60, 100);
l = linspace(150, 20, 19); l2 = ones(1,40)*20; l3 = horzcat(l,l2);
for i= 2:59
    a(i,2:60) = l3;
end
aa = a/365/86400*917;

fid = fopen('mass1.bin','w','b'); fwrite(fid,aa,'real*8'); fclose(fid);
%%
clear
a = zeros(60, 100);
l = linspace(150, 20, 19); l2 = ones(1,40)*20; l3 = horzcat(l,l2);
...
aa = a/365/86400*917*2;

fid = fopen('mass2.bin','w','b'); fwrite(fid,aa,'real*8'); fclose(fid);
...
% fid = fopen('Hinit60y.box','r','b'); H_streamice = fread(fid,[nx ny],'real*8'); fclose(fid);
