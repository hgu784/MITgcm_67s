...
t = readbin('initT_latlon_qsg.bin',[460 600 130],1,'real*4'); tt = imresize3(t,[115 150 130],'nearest');
s = readbin('initS_latlon_qsg.bin',[460 600 130],1,'real*4'); ss = imresize3(s,[115 150 130],'nearest');

fid=fopen('theta2.init','w','b');fwrite(fid,tt,'real*4');fclose(fid);
fid=fopen('salt2.init','w','b');fwrite(fid,ss,'real*4');fclose(fid);

%% Set the grid size;
clear
nx = 115;    delx = 1;   X = nx*delx;
ny = 150;    dely = 1;   Y = ny*dely;
nz = 130;    delz = 10;  Z = nz*delz;
...
fid=fopen('vvel.obw','w','b');  fwrite(fid,V_OBC,'real*4'); fclose(fid);
% fid=fopen('uvel.obw','w','b');  fwrite(fid,U_OBC,'real*4'); fclose(fid);
fid=fopen('theta.obw','w','b');  fwrite(fid,T_OBC,'real*4'); fclose(fid);
fid=fopen('salt.obw','w','b');  fwrite(fid,S_OBC,'real*4'); fclose(fid);
...
fid=fopen('theta.init','w','b');fwrite(fid,T_init,'real*4');fclose(fid);
fid=fopen('salt.init','w','b');fwrite(fid,S_init,'real*4');fclose(fid);
...
% fclose(fid_S);
%%
a = readbin('/home/toshiki/MITgcm/run/jim_run3/input/theta.obw', [60 55], 1, 'real*4');
%%
figure
quiver()