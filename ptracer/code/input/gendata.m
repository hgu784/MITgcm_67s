...
z = zeros(60, 100,55);
fid = fopen('zero.bin','w','b'); fwrite(fid,z,'real*8'); fclose(fid);
fid = fopen('ini_tr1.bin.data','w','b'); fwrite(fid,z,'real*8'); fclose(fid);
%%
su = rdmds('/media/toshiki/HDD_hirata1/run/jim_run3/run/surfDiag.0018662400');
s = su(:,:,6);
fid = fopen('Hinit_new.box','w','b'); fwrite(fid,s,'real*8'); fclose(fid);
...
fid = fopen('Hinit_new.box','r','b'); H_streamice = fread(fid,[nx ny],'real*8'); fclose(fid);
