...
H = zeros(240, 400);
h = rdmds('/home/toshiki/MITgcm/run2/jim_run10/input/surfDiag.0018921600');
h = h(2:59,2:60, 6);
h = imresize(h, [238 239]);
H(2:239, 2:240) = h; H([1 end], 1:240) = 400; H(:, 1) = 400;
fid = fopen('H_new1.box','w','b'); fwrite(fid,H,'real*8'); fclose(fid);

aa = ones(240, 240); bb = zeros(240, 160);
calve = horzcat(aa,bb);
fid = fopen('IM_CALVE60.box','w','b'); fwrite(fid,calve,'real*8'); fclose(fid);
%%
% Dimensions of grid
nx=240; 
ny=400;
...
fid = fopen('H_new.box','r','b'); H_streamice = fread(fid,[nx ny],'real*8'); fclose(fid);
...
x = 0 : (5*pi)/239 : 5*pi; hin = 1100 + 100*sin(x);
y = 0 : (200*pi)/239 : 200*pi ; Hin = hin + (30*sin(y));
% Hin = 1200;
Hbc = zeros(ny,nx);
Hbc(2,:) = Hin;
...
fid = fopen('H_new1.box','r','b');
VV = fread(fid,inf,'real*8');
fclose(fid);

VV=reshape(VV, [240 400]);
