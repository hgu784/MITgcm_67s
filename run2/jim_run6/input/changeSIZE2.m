...
h = H(2:59, 2:60);
J = imresize(h,[238 239]); % (60*60 to 240*240)
one = ones(238, 1)*400; one2 = ones(1, 240)*400; zero = zeros(240, 160);
...
vd = VD(:, 2); vd = imresize(vd, [240 1]);
VD2 = zeros(240, 400); VD2(:,2) = vd;
...
HBC2 = zeros(240, 400); HBC2(:, 2) = 1200;
...
hm1 = ones(240, 240); hm0 = zeros(240, 160);
...
e = e0(2:59, 2:60);
K = imresize(e,[238 239]); % (60*60 to 240*240)
one = ones(238, 1)*3.2688; one2 = ones(1, 240)*3.2688; zero = zeros(240, 160);
...
tp = topo(2:59, 2:60);
L = imresize(tp,[238 239], 'box');
one = ones(238, 1)*-360; one2 = ones(1, 240)*-360; zero = zeros(240, 160);
...
et = eta(2:59, 2:60);
K = imresize(et,[238 239]); % (60*60 to 240*240)
one = ones(238, 1)*3.2688; one2 = ones(1, 240)*3.2688; zero = zeros(240, 160);
...
ma = mass(2:59, 2:60);
I = imresize(ma,[238 239]); % (60*60 to 240*240)
one = ones(238, 1)*3661600; one2 = ones(1, 240)*3661600; zero = zeros(240, 160);
...
im1 = ones(240, 240); im0 = zeros(240, 160);
IM2 = horzcat(im1, im0);
