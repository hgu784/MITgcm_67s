...
c = 0 : (2*pi)/11 : 2*pi; cline = -10*sin(c)+15;
T_OBC(1:nx,1:nz,1:12) = NaN;
S_OBC(1:nx,1:nz,1:12) = NaN;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for iz = 1:nz
        T_OBC(1:nx,iz,k) = tref(iz);
    end
    
    for iz = 1:nz
        S_OBC(1:nx,iz,k) = sref(iz);
    end
    
k = k+1;    
end
%%
TT = cat(3,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC,T_OBC);
TTT = cat(3,TT,TT,TT,TT,TT,TT);

SS = cat(3,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC,S_OBC);
SSS = cat(3,SS,SS,SS,SS,SS,SS);
for i = 1:12
    figure(88)
    mp(TT(:,:,i))
    title(i)
...
%%
fid=fopen('theta_season.obw','w','b');  fwrite(fid,TTT,'real*8'); fclose(fid);
fid=fopen('salt_season.obw','w','b');  fwrite(fid,SSS,'real*8'); fclose(fid);
